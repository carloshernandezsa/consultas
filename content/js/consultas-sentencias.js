...
if (autoridad_clave == null) {
  setTimeout(function () {
    ...
    $("#sentenciasFormCard").show(); // Mostrar el formulario
    $("#spinnerCard").hide(); // Ocultar el spinner
  }, 1000); // Esperar un segundo
} else {
  setTimeout(function () {
    consultarSentencias(autoridad_clave); // Consultar las sentencias
    $("#sentenciasTableCard").show(); // Mostrar la tabla
    $("#spinnerCard").hide(); // Ocultar el spinner
  }, 1000); // Esperar un segundo
}
...
function consultarSentencias(autoridad_clave) {
  ...
  fetch(url + "/autoridades/" + autoridad_clave)
    ...
    .then((data) => {
      ...
      if (data.success === true) {
        $("#sentenciasTableHeader").text(data.descripcion);
      }
    })
    .catch((error) => console.log(error));
}
