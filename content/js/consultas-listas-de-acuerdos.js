...
if (autoridad_clave == null) {
  setTimeout(function () {
    ...
    $("#listasDeAcuerdosFormCard").show(); // Mostrar el formulario
    $("#spinnerCard").hide(); // Ocultar el spinner
  }, 1000); // Esperar un segundo
} else {
  setTimeout(function () {
    consultarListasDeAcuerdos(autoridad_clave); // Consultar las listas de acuerdos
    $("#listasDeAcuerdosTableCard").show(); // Mostrar la tabla
    $("#spinnerCard").hide(); // Ocultar el spinner
  }, 1000); // Esperar un segundo
}
...
function consultarListasDeAcuerdos(autoridad_clave) {
  ...
  fetch(url + "/autoridades/" + autoridad_clave)
    ...
    .then((data) => {
      ...
      if (data.success === true) {
        $("#listasDeAcuerdosTableHeader").text(data.descripcion);
      }
    })
    .catch((error) => console.log(error));
}
